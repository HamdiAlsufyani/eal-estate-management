<x-admin-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="text-sm text-text-muted">Overview of your real estate operations.</p>
        </div>
    </x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <x-ui.stat-card label="Pending Properties" :value="number_format($stats['pending_properties'])" variant="warning">
                <x-slot name="icon">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">
            ...
            <x-ui.stat-card label="Approved Properties" :value="number_format($stats['approved_properties'])" variant="success">
                <x-slot name="icon">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">
            ...
            <x-ui.stat-card label="Rejected Properties" :value="number_format($stats['rejected_properties'])" variant="danger">
                <x-slot name="icon">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">
            ...
            <x-ui.stat-card label="Total Users" :value="number_format($stats['total_users'])" variant="primary">
                <x-slot name="icon">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">
            ...
            <x-ui.stat-card label="Pending Users" :value="number_format($stats['pending_users'])" variant="warning">
                <x-slot name="icon">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">
            ...
            <x-ui.stat-card label="Property Views" :value="number_format($stats['property_views'])" variant="info">
                <x-slot name="icon">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">
        ...
        {{-- Latest Properties --}}
        <x-ui.card title="Latest Properties">
            @if ($latestProperties->isEmpty())
                <x-ui.empty-state title="No properties yet" description="New property listings will appear here as they are submitted." />
            @else
                <x-ui.table>
                    <x-slot name="head">
                        <th>Title</th>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            {{-- Latest Inquiries --}}
            <x-ui.card title="Latest Inquiries">
                @if ($latestInquiries->isEmpty())
                    <x-ui.empty-state title="No inquiries yet" />
                @else
                    <x-ui.table>
                        <x-slot name="head">
                            <th>Property</th>
