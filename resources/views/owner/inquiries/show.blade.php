...
<x-owner-layout title="{{ __('inquiries.inquiry_details') }}" :breadcrumbs="[['label' => __('navigation.inquiries'), 'url' => route('owner.inquiries.index')], ['label' => '#' . $inquiry->id]]">
    <x-slot name="header">
        <div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="space-y-6 lg:col-span-2">
                <x-ui.card title="{{ __('inquiries.property') }}">
                    @if ($inquiry->property)
                        <a href="{{ route('owner.properties.show', $inquiry->property) }}" class="font-medium text-primary hover:underline">
            ...
            <div class="space-y-6">
                <x-ui.card title="{{ __('messages.status') }}">
                    ...
                    @can('update', $inquiry)
                        <form method="POST" action="{{ route('owner.inquiries.status', $inquiry) }}" class="mt-4 space-y-3">
                            @csrf
                            @method('PATCH')
                            <x-ui.select
                                name="status"
