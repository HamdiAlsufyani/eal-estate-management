<x-admin-layout title="{{ __('navigation.amenities') }}" :breadcrumbs="[['label' => __('navigation.amenities')]]">
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            ...
            @can('create', \App\Models\Amenity::class)
                <x-ui.button :href="route('admin.amenities.create')" variant="primary" class="hidden sm:inline-flex">
    ...
    <div x-data="{ confirmTarget: null }" class="space-y-6">
        ...
        <x-ui.card>
            @include('admin.amenities.partials.filters')

            @include('admin.amenities.partials.table', ['amenities' => $amenities])
        </x-ui.card>
