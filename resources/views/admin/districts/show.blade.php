<x-admin-layout title="{{ $district->name }}" :breadcrumbs="[['label' => 'Districts', 'url' => route('admin.districts.index')], ['label' => $district->name]]">
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-xl font-semibold text-text">{{ $district->name }}</h1>
                <p class="text-sm text-text-muted">District details and usage.</p>
            </div>

            <div class="hidden items-center gap-2 sm:flex">
                @can('update', $district)
                    <x-ui.button :href="route('admin.districts.edit', $district)" variant="outline" size="sm">Edit</x-ui.button>
                @endcan
            </div>
        </div>
    </x-slot>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-6 lg:col-span-2">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-1">
                    <x-ui.stat-card label="Properties" :value="number_format($district->properties_count)" variant="primary">
                        <x-slot name="icon">
                ...
                <x-ui.card title="Actions">
                    <div class="flex flex-wrap gap-3">
                        @can('update', $district)
                            <x-ui.button :href="route('admin.districts.edit', $district)" variant="outline" size="sm">Edit District</x-ui.button>
                        @endcan

                        @can('delete', $district)
                            @if ($district->properties_count > 0)
                                <p class="text-sm text-text-muted">
                                    This district has {{ number_format($district->properties_count) }} propert{{ $district->properties_count === 1 ? 'y' : 'ies' }} assigned and cannot be deleted.
                                </p>
                            @else
                                <x-ui.button
                                    type="button"
        ...
        @can('delete', $district)
            @if ($district->properties_count === 0)
                <x-ui.modal name="delete-district" max-width="md">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-text">Delete District</h3>
                        <p class="mt-2 text-sm text-text-muted">
                            Are you sure you want to delete <span class="font-medium text-text">{{ $district->name }}</span>? This action cannot be undone.
                        </p>

                        <div class="mt-6 flex justify-end gap-3">
                            <x-ui.button type="button" variant="outline" @click="$dispatch('close-modal', 'delete-district')">Cancel</x-ui.button>
