<x-admin-layout title="{{ __('navigation.property_types') }}" :breadcrumbs="[['label' => __('navigation.property_types')]]">
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-xl font-semibold text-text">{{ __('navigation.property_types') }}</h1>
                <p class="text-sm text-text-muted">{{ __('properties.property_types_subtitle') }}</p>
            </div>

            @can('create', \App\Models\PropertyType::class)
                <x-ui.button :href="route('admin.property-types.create')" variant="primary" class="hidden sm:inline-flex">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 w-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                    {{ __('properties.new_property_type') }}
                </x-ui.button>
            @endcan
        </div>
    </x-slot>
    ...
    <div x-data="{ confirmTarget: null }" class="space-y-6">
        ...
        <x-ui.card>
            @include('admin.property-types.partials.filters')

            @include('admin.property-types.partials.table', ['propertyTypes' => $propertyTypes])
        </x-ui.card>
        ...
        <x-ui.modal name="delete-confirm" max-width="md">
            <div class="p-6">
                ...
                <h3 class="mt-4 text-lg font-semibold text-text">{{ __('properties.delete_property_type') }}</h3>
